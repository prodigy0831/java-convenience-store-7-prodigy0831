...
public class InputHandlerTest {
    ...
    private static final String ERROR_NOT_FOUND_PRODUCT = "[ERROR] 존재하지 않는 상품입니다. 다시 입력해 주세요.";
    private static final String ERROR_OUT_OF_STOCK = "[ERROR] 재고 수량을 초과하여 구매할 수 없습니다. 다시 입력해 주세요.";
    ...
    void EmptyOrNullInputTest(String input) {
        assertThatThrownBy(() -> inputHandler.processInput(input))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining(ERROR_INVALID_FORMAT);
    }
    ...
    void NotFoundProductNameTest(String input) {
        assertThatThrownBy(() -> inputHandler.processInput(input))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining(ERROR_NOT_FOUND_PRODUCT);
    }
    ...
    void OutOfStockTest(String input) {
        assertThatThrownBy(() -> inputHandler.processInput(input))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining(ERROR_OUT_OF_STOCK);
    }

    @ParameterizedTest
    @ValueSource(strings = {"[콜라-10][콜라-11]"})
    @DisplayName("유효한 괄호이나 쉼표가 없는 경우 오류 출력")
    void InvalidFormatDoubleInputTest(String input) {
        assertThatThrownBy(() -> inputHandler.processInput(input))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining(ERROR_INVALID_FORMAT);
    }
